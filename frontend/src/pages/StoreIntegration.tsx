...
const StoreIntegration: React.FC = () => {
  ...
  const loadNearbyStores = async () => {
    try {
      ...
      setNearbyStores(response.data.data?.stores || []);
    } catch (error) {
  ...
  const loadPriceComparison = async () => {
    try {
      ...
      setPriceComparison(response.data.data);
    } catch (error) {
  ...
  const loadStoreRecommendations = async () => {
    try {
      const response = await apiService.post('/api/stores/recommendations', {
        shopping_list: shoppingList,
        preferences: { priority: 'price', max_distance: maxDistance }
      });
      setStoreRecommendations(response.data.data?.recommendations || []);
    } catch (error) {
  ...
  const loadCurrentDeals = async () => {
    try {
      ...
      setCurrentDeals(response.data.data?.deals_by_store || []);
    } catch (error) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <TabPanel value={tabValue} index={0}>
        <Typography variant="h6" gutterBottom>Nearby Stores</Typography>
            ...
            >
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                  ...
                  <Chip icon={<StarIcon />} label={store.ratings} size="small" color="primary" />
                </Box>
                <Typography color="text.secondary" gutterBottom>
                  <LocationIcon sx={{ fontSize: 16, mr: 0.5 }} />
                  {store.address}
                </Typography>
                <Typography variant="body2">Distance: {store.distance} miles</Typography>
                <Box sx={{ mt: 1 }}>
                  <Chip 
      ...
      <TabPanel value={tabValue} index={2}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 2 }}>
          {storeRecommendations.map((recommendation) => (
            <Card key={recommendation.store_id}>
              <CardContent>
                ...
                {recommendation.pros && recommendation.pros.length > 0 && (
                  <Box sx={{ mb: 1 }}>
                    ...
                    {recommendation.pros.map((pro: string, index: number) => (
                      <Typography key={index} variant="body2" sx={{ ml: 2 }}>• {pro}</Typography>
                    ))}
                  </Box>
                )}
                ...
                {recommendation.cons && recommendation.cons.length > 0 && (
                  <Box>
                    ...
                    {recommendation.cons.map((con: string, index: number) => (
                      <Typography key={index} variant="body2" sx={{ ml: 2 }}>• {con}</Typography>
                    ))}
                  </Box>
                )}
              </CardContent>
      ...
      <TabPanel value={tabValue} index={3}>
        <Typography variant="h6" gutterBottom>Current Deals & Promotions</Typography>

        {currentDeals.map((storeDeals) => (
          <Card key={storeDeals.store_id} sx={{ mb: 2 }}>
            <CardContent>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 2 }}>
                {storeDeals.deals.map((deal: any, index: number) => (
                  <Card key={index} variant="outlined">
                    <CardContent>
                      <Typography variant="h6" color="primary">{deal.item_name}</Typography>
                      <Typography color="text.secondary" gutterBottom>{deal.category}</Typography>
                      <Typography variant="h5">{formatCurrency(deal.price)}</Typography>
                      <Typography variant="body2">per {deal.unit}</Typography>
                      <Box sx={{ mt: 1 }}>
                        <Chip
